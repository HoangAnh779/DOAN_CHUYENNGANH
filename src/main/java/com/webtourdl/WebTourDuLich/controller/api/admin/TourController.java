...
import com.webtourdl.WebTourDuLich.dto.ToutStartAddDTO;
...
import com.webtourdl.WebTourDuLich.utilities.DateUtils;
...
public class TourController {
    ...
    @GetMapping("/count")
    public ResponseDTO getTourCount() {
        long tourCount = tourService.countTours();
        Map<String, Long> response = new HashMap<>();
        response.put("count", tourCount);
        return new ResponseDTO("Thành công", response);
    }

    @PostMapping("/apply-discount/{id}")
    public ResponseDTO applyDiscount(@PathVariable("id") Long tourId,
                                     @RequestParam("discountCode") String discountCode) {
        TourDTO tour = tourService.findTourById(tourId);
        if (tour == null) {
            return new ResponseDTO("Tour không tồn tại", null);
        }
        if (discountCode.equalsIgnoreCase(tour.getMa_giam_gia())) {
            Integer discountPercent = tour.getGiam_gia();
            return new ResponseDTO("Mã giảm giá hợp lệ", discountPercent);
        }
        return new ResponseDTO("Mã giảm giá không hợp lệ", null);
    }
