...
public class TourServiceImpl implements TourService {
	...
	public Tour addTour(TourDTO tourDTO) {
		...
		tour.setGiam_gia(tourDTO.getGiam_gia());
		tour.setMa_giam_gia(tourDTO.getMa_giam_gia());
	...
	public Tour updateTour(TourDTO newTour, Long id) {
		...
		if(tour.isPresent()) {
			...
			updatedTour.setGiam_gia(newTour.getGiam_gia());
			updatedTour.setMa_giam_gia(newTour.getMa_giam_gia());
	...
	public Long calculateDiscountedPrice(Long originalPrice, Integer discountPercent) {
	...
	public Tour applyDiscount(Long tourId, String discountCode) {
		Tour tour = tourRepository.findById(tourId).orElse(null);
		if (tour == null) {
			throw new IllegalArgumentException("Tour không tồn tại");
		}

		if (discountCode != null && discountCode.equalsIgnoreCase(tour.getMa_giam_gia())) {
			Integer discount = tour.getGiam_gia();
			Long discountedPrice = calculateDiscountedPrice(tour.getGia_tour(), discount);
			tour.setGia_tour(discountedPrice);
		}
		return tour;
	}
}
